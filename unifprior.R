...
for (k in 1: K)
{
  ...
  for (m in 2:M)
  {
    ...
    for (j in 1:J)
    {
      Tau[j,m,k]=rtruncgamma(1,a=a,b=b,shape=1+sum(is.na(z[,j])==F)/2,
                        rate=sum((Z[,j]-Theta[,m,k])^2,na.rm=T)/2)
    }
    for (l in 1:10)
    {
      for (j in 1:J)
  ...
  cor_10_Bayes[k]=cor(true_rank[true_rank[,k]<=10,k],Bayes_rank[true_rank[,k]<=10,k]) ##wrong
...
save.image(file = "D:/research/Bayes/unif.RData")
setwd("D:/research/Bayes")
load("unif.RData")
pdf("unifTrace.pdf",width=12)
...
dev.off()
pdf("D:\\Dropbox\\Research_Lily_Sherry\\Report\\Presentations\\unifTrace_p.pdf",width=8)
par(mfrow=c(3,1),mai=c(0.3,0.6,0.35,0.2))
plot(Theta[1,,1],type="l",main="Trace Plots from Uniform Prior")
